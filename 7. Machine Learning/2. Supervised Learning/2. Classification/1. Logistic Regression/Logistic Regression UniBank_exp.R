...
log.model =  glm(Personal.Loan ~ Income, family = binomial(), bankdata)
summary(log.model)

# Prob(Personal.Loan / Income = x) = 1/(1+ exp-(B0+B1x))
#    Odds(Personal.Loan = Yes)     = exp(B0+B1x)

# Prob = 1/(1+exp(6.12-0.03*Income) )
...
table(train_set$Personal.Loan, pred_train)
...
fun_cutoff = function (df_resutl, cutoff) {
    ...
    cm_cutoff = table(train_set$Personal.Loan, y_pred_cutoff)
...
table(test_set$Personal.Loan, pred_test)
...
dt = lift(set_prob$Personal.Loan  , set_prob$pred_test, groups = 10)
dt
...
set_prob$Personal.Loan = as.numeric(as.character(set_prob$Personal.Loan))
gain = gains(actual = set_prob$Personal.Loan, predicted = set_prob$score_test)
print.gains(gain)
...
pred = prediction(set_prob$score_test , set_prob$Personal.Loan)
...
summary(test_set$Personal.Loan)
...
# Prediction function
ROCRpred = prediction(predictTrain, train_set$Personal.Loan)
...
confusionMatrix(test_set$Personal.Loan, pred_test)

str(test_set$Personal.Loan)
str(pred_test)
