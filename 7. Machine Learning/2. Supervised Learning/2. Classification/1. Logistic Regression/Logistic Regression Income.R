...
setwd('C:/Eminent')
demodata = read.csv('adult.csv')
dim(demodata)
str(demodata)
View(demodata)
table(demodata$workclass)
demodata$workclass = as.character(demodata$workclass)

demodata$workclass[demodata$workclass == 'Without-pay']

demodata$workclass[demodata$workclass == 'Without-pay' | demodata$workclass == 'Never-worked'] = "Unemployed"

demodata$workclass[demodata$workclass == 'State-gov' | demodata$workclass == 'Local-gov'] = 'SL-gov'

demodata$workclass[demodata$workclass == 'Self-emp-inc' |
                    demodata$workclass == 'Self-emp-not-inc'] = 'Self-employed'

table(demodata$workclass)

table(demodata$marital.status)
demodata$marital.status = as.character(demodata$marital.status)
demodata$marital.status[ demodata$marital.status %in% c('Married-AF-spouse','Married-civ-spouse','Married-spouse-absent')] = 'Married'
demodata$marital.status[ demodata$marital.status %in% c('Divorced','Separated','Widowed')] = 'Not-Married'


demodata$native.country = as.character(demodata$native.country)
table(demodata$native.country)
...
demodata$native.country[demodata$native.country %in% north.america] = "North America"
demodata$native.country[demodata$native.country %in% asia] = "Asia"
demodata$native.country[demodata$native.country %in% south.america] = "South America"
demodata$native.country[demodata$native.country %in% europe] = "Europe"
demodata$native.country[demodata$native.country %in% other] = "Other"


demodata$native.country = as.factor(demodata$native.country)
demodata$marital.status = as.factor(demodata$marital.status)
demodata$workclass = as.factor(demodata$workclass)
str(demodata)

table(demodata$education)

table(demodata$workclass)
demodata[demodata == '?'] = NA
 
table(demodata$income) 

library(Amelia)
AmeliaView()
missmap(demodata, y.at = 1, y.labels = "", col = c("yellow", "black"), legend = FALSE)

result_na = sapply(demodata, function(df) {sum(is.na(df) *100)/ length(df)}) 
...
#View(result_na_df)
#demodata = demodata[ , result_na_df$column]

# Ommit NAs

demodata = na.omit(demodata)
...
library(ggplot2)
ggplot(demodata, aes(age)) + geom_histogram(aes(fill = income), color = "black",
                                         binwidth = 1)



demodata$income  = factor(demodata$income,
                                  levels = c('<=50K','>50K'),
                                  labels = c('0','1') )
...
library(caTools)
set.seed(123)
split = sample.split(demodata$income, SplitRatio = 0.7)
train_set = subset(demodata, split == TRUE)
test_set  = subset(demodata, split == FALSE)

dim(train_set)
dim(test_set)


log.model =  glm(income ~ ., family = binomial(), train_set)
summary(log.model)
