...
#another way
ggplot(data, aes(x = data$conservation, y = data$awake)) + geom_point()
# and yet another way
ggplot(mapping = aes(data$conservation, data$awake)) + geom_point()
...
# Change the colors
plot + geom_point(aes(color = log(brainwt))) + 
  scale_color_gradient2(low = "green", mid = "yellow", high = "firebrick", 
...
plot + geom_point(aes(alpha = sleep_rem)) + 
  xlab("this is our x axis") + ylab("this is our y axis") +
  ggtitle("title") + scale_alpha("our legend")
...
plot + geom_point() + facet_wrap(~vore, nrow = 2, ncol = 4)
...
# size mapped to the log of brain weight ( log(data$brainwt) ). Bonus points if
# you add axis labels and a title.

ggplot(data, aes(x = conservation, y = awake, color = vore, size = log(brainwt))) +
  geom_point() + xlab("x label") + ylab("y label") + ggtitle("blaskdjflaksjd")
...
# huh, group was actually unneccesary
ggplot(data, aes(x = bodywt, y = brainwt, color = vore)) +
  geom_line() + scale_x_log10() + scale_y_log10()
...
# to show difference between scatterplot
ggplot(data, aes(x = vore, y = sleep_total)) + geom_point()
...
ggplot(data, aes(x = conservation, y = sleep_total, fill = conservation)) + geom_boxplot()
...
# CHANGE BACKGROUND
plot <- ggplot() + geom_bar(aes(x = names(sleepMeans), y = sleepMeans, fill = names(sleepMeans)), stat = "identity")
plot + theme_bw()

library(ggthemes)
plot + theme_wsj()

#This is where we stopped during the tutorial (although we did skip down to the
#bottom to look at saving images!) 
#######################################################################
...
ggsave(filename = "example.png", width = 10, height = 10, units = "cm", dpi = 300)
...
# So yeah, ggplot2 is a pretty powerful (but tricky) package. To see what's
# possible, read the documentation at: http://docs.ggplot2.org/current/
...
# Solution: Add some geometry, like geom_point().

# Error: geom_whatever is missing required aesthetics: aethetic1, aesthetic2.
# Solution: For whatever reason, your mappings for required aesthetics are not 
# getting passed to the geometries where they are needed. Make sure you've
# specified the required mappings for all layers!

# Error: ggplot does not know how to interpret data of class uneval.
# Solution: ggplot() has two arguments (ggplot(data, mapping)). If you are not 
# specifying any data, ggplot will assume that your aesthetics are the data 
# argument and try to interpret things that way. You can get around this by
# specifying ggplot(mapping = yourAesthetics()) instead of ggplot(yourAesthetics()).
